	...
	unsigned char image_bw[Y_SIZE][X_SIZE])
{
	...
	for (y = 0; y < (int)biHeight; y++)
		for (x = 0; x < (int)biWidth; x++) {
			a = (int)(0.3 * image[biHeight - 1 -y][x][0] + 0.59 * image[biHeight - 1 - y][x][1] + 0.11 * image[biHeight - 1 - y][x][2]);
			if (a < LOW) a = LOW;
			if (a > HIGH) a = HIGH;
			image_bw[y][x] = a;
		}
	...
	unsigned char image[Y_SIZE][X_SIZE][3])
{
	...
	for (y = 0; y < (int)biHeight; y++)
		for (x = 0; x < (int)biWidth; x++) {
			...
			image[biHeight - 1 - y][x][0] = a;
			image[biHeight - 1 - y][x][1] = a;
			image[biHeight - 1 - y][x][2] = a;
		}
}
